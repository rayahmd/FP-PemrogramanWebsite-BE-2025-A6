...
import { CreateGameshowDTO, CheckGameshowAnswerDTO, GameshowParameters, GameshowQuestion } from './schema';
  ...
  checkAnswer: async (gameId: string, payload: CheckGameshowAnswerDTO) => {
    ...
    if (isCorrect) {
      ...
      if (payload.timeTaken !== undefined) {
        ...
        const timeRatio = Math.max(0, (question.timeLimit - payload.timeTaken) / question.timeLimit);
        // Rumus: Base 50% poin + (0-50% bonus berdasarkan kecepatan)
        earnedPoints = Math.round((maxPoints * 0.5) + (maxPoints * 0.5 * timeRatio));
      } else {
